...
from flux.model import Flux, FluxParams, FluxLoraWrapper
...
def main(args):
    ...
    with torch.device(device):
        ref_model = Flux(config.params).to(torch.bfloat16)
    ...
    missing, unexpected = ref_model.load_state_dict(sd, strict=False, assign=True)
    ...
    for name, param in model.named_parameters():
        if args.use_lora and 'lora' in name:
    ...
    for step in range(resume_step, args.max_steps):
        ...
        for mb_idx in range((data_pack["local_bsz"] - 1) // data_pack["micro_bsz"] + 1):
            ...
            with {
                ...
                loss_dict = data_pack["transport"].training_losses_lora(model, x_mb, ref_model, model_kwargs, extra_kwargs, data_pack["local_bsz"], step)
