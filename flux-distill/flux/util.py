...
def load_flow_model(name: str, device: str | torch.device = "cuda", dtype=torch.float32, hf_download: bool = True, attn_token_select: bool = False, mlp_token_select: bool = False, zero_init: bool = False, use_lora: bool = False):
    # Loading Flux
    print("Init model")
    ...
    with torch.device(device):
        if use_lora:
            model = FluxLoraWrapper(params=params).to(dtype)
        else:
