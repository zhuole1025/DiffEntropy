...
class Attention(nn.Module):
    ...
    def forward(
        ...
        attn_weight = xq.permute(0, 2, 1, 3) @ xk.permute(0, 2, 1, 3).transpose(-2, -1) * scale_factor
        attn_weight = torch.softmax(attn_weight, dim=-1)
        
        # compute the entropy of the attention weights
        entropy = torch.sum(-attn_weight * torch.log(attn_weight + 1e-6), dim=-1).mean()
        
        output = output.flatten(-2)

        return self.wo(output), entropy.detach().cpu()
...
class TransformerBlockSandwichNorm2(nn.Module):
    ...
    def forward(
        ...
        if adaln_input is not None:
            ...
            h = x + gate_msa.unsqueeze(1).tanh() * self.attention_norm(attn_out)
...
class DiT_Llama(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        for layer in self.layers:
            x, e = layer(x, freqs_cis, adaln_input=adaln_input)
            entropy.append(e)
        
        # save entropy
        entropy = torch.stack(entropy).unsqueeze(0)
        import os
        time = round(time, 4)
        if os.path.exists(f"results/entropy_{time}.pt"):
            tmp = torch.load(f"results/entropy_{time}.pt")
            entropy = torch.cat([tmp, entropy], dim=0)
        torch.save(entropy, f"results/entropy_{time}.pt")
